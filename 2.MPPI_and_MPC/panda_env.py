...
class PandaEnv(BaseEnv):
    ...
    def _get_state_space(self):
        state_space = gym.spaces.Box(low=np.concatenate([self.joint_limits[0], self.vel_limits[0]]).astype(np.float32),
                                     high=np.concatenate([self.joint_limits[1], self.vel_limits[1]]).astype(np.float32),
                                     )
        return state_space

    def _get_action_space(self):
        action_space = gym.spaces.Box(low=self.joint_torque_limits[0].astype(np.float32),
                                      high=self.joint_torque_limits[1].astype(np.float32))
        return action_space
